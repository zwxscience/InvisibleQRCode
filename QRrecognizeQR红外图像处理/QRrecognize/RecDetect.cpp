...
RecDect::RecDect(IplImage *dst){
	...
	img = cvCloneImage( dst );
	//转化灰度
	img0=cvCreateImage(cvGetSize(img),img->depth,1);//灰度
	cvSetImageCOI(img,1);//B色
	cvCopy(img,img0);  //提取B色
	cvSetImageCOI(img,0);//恢复
		//cvShowImage("111",img0);
	//cvThreshold(img0,img0,otsu(img0),255,1);
cvAdaptiveThreshold(img0,img0,255,CV_ADAPTIVE_THRESH_GAUSSIAN_C,CV_THRESH_BINARY_INV,101,-5);//-数越大，细节保存越不清楚
...
CvSeq* RecDect::findSquares4()
{
	...
	cvAdaptiveThreshold(img2,img2,255,CV_ADAPTIVE_THRESH_GAUSSIAN_C,CV_THRESH_BINARY_INV,101,-11);
	cvCanny(img2,img2,50,150,3); 
	cvSmooth(img2,img2);
	cvShowImage("ewe",img2);
	cvWaitKey(0);
	cvAdaptiveThreshold(img2,img2,255,CV_ADAPTIVE_THRESH_GAUSSIAN_C,CV_THRESH_BINARY_INV,101,-7);
...
void RecDect::RfindSquaresRect(float low,float high)
{
	...
	cvCanny(img2,img2,50,150,3); 
	cvSmooth(img2,img2);
	cvAdaptiveThreshold(img2,img2,255,CV_ADAPTIVE_THRESH_GAUSSIAN_C,CV_THRESH_BINARY_INV,201,-11);
...
int RecDect::recognVersion(CvSeq* squares)
{
	...
	for(int i = 0; i < squares->total; i += 4 )
	{
		...
		if(maxPoint(temp[0].x,temp[1].x,temp[2].x,temp[3].x)<0.5*img->width&&maxPoint(temp[0].y,temp[1].y,temp[2].y,temp[3].y)<0.5*img->height)//左上角
		{
			...
			{//第一次占用
				...
				if ((areaold>areanew&&dist<img->width/4)||fabs((float)corner1[1].x-pt[1].x)>0.12*img->width||fabs((float)corner1[1].y-pt[1].y)>0.12*img->width)//如果计算的面积比原来的小，则替换掉
				{
					for ( int index = 0; index < 4; index++ )
						corner1[ index ] =temp[ index ];
				}
		...
		if(minPoint(temp[0].x,temp[1].x,temp[2].x,temp[3].x)>0.5*img->width&&maxPoint(temp[0].y,temp[1].y,temp[2].y,temp[3].y)<0.5*img->height)//右上角
		{
			...
			{//第一次占用
				...
				float areaold=edge3*edge4;//计算存入的面积
				if ((areaold>areanew&&dist<img->width/4)||fabs((float)-corner2[0].x+pt[0].x)>0.12*img->width||fabs((float)-corner2[0].y+pt[0].y)>0.12*img->width)//如果计算的面积比原来的小，则替换掉
				{
					for ( int index = 0; index < 4; index++ )
						corner2[ index ] =temp[ index ];
				}
		...
		if(maxPoint(temp[0].x,temp[1].x,temp[2].x,temp[3].x)<0.5*img->width&&minPoint(temp[0].y,temp[1].y,temp[2].y,temp[3].y)>0.5*img->height)//左下角
		{
			...
			{//第一次占用
				...
				float areaold=edge3*edge4;//计算存入的面积
				if ((areaold>areanew&&dist<img->width/4)||fabs((float)corner3[2].x-pt[2].x)>0.12*img->width||fabs((float)-corner3[2].y+pt[2].y)>0.12*img->width)//如果计算的面积比原来的小，则替换掉
				{
					for ( int index = 0; index < 4; index++ )
						corner3[ index ] =temp[ index ];
				}
		...
		if(minPoint(temp[0].x,temp[1].x,temp[2].x,temp[3].x)>0.5*img->width&&minPoint(temp[0].y,temp[1].y,temp[2].y,temp[3].y)>0.5*img->height)//右下角
		{
			...
			{//第一次占用
				...
				float areaold=edge3*edge4;//计算存入的面积
				if ((areaold>areanew&&dist<img->width/4)||fabs((float)-corner4[3].x+pt[3].x)>0.12*img->width||fabs((float)-corner4[3].y+pt[3].y)>0.12*img->width)//如果计算的面积比原来的小，则替换掉
	...
	WUL=cvRound(7.0/3*WUL);
	WUR=cvRound(7.0/3*WUR);
	//得到以上信息后就可以进行版本确定了	
...
{
	IplImage* src=cvCloneImage( Img_old );
...
void RecDect::drawGrid()//图像归一化
{
	...
	IplImage *QRImage=cvCreateImage(QRSize,img0->depth,img0->nChannels);
	CvRect QRcontent=cvRect(pt[1].x,pt[1].y,pt[0].x-pt[1].x,pt[2].y-pt[1].y);
...
if (count==number-1)
{
	range=6;//取滑动范围
		...
		{
			if(GetNonZero(canyImage,t,0)<GetNonZero(canyImage,line[i],0))
				t=line[i];
		}	  
	...
	for(int i=modeWidth+starty;count<number;i=cols[count+1]+modeWidth,count++){
		...
		for (int i=0;i<line.size();i++)
		{
			if(GetNonZero(canyImage,t,1)<GetNonZero(canyImage,line[i],1))
				t=line[i];
		}	  
		line.clear();//每循环一次，释放一次
...
void RecDect::AreaRotate()//获取QR图像所在的区域
{
	...
	if(fabs(q)<0.005||fabs(fabs(q)-CV_PI/2.0)<0.005){	
		cvSetImageCOI(img,1);//绿色
	cvCopy(img,img0);  //提取绿色
	cvSetImageCOI(img,0);//恢复
	...
	RfindSquaresRect(0.1f,0.9);//获取图像轮廓定点坐标:此步获取图像大概位置
	
	//cvShowImage("截取后旋转的图像",img);
